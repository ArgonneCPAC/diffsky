...
from ...param_utils import diffsky_param_wrapper as dpwm
from .. import plot_cosmos_merging as plcm
...
def _generate_testing_data(seed=0):
    testing_data = plcm.get_plotting_data(
...
def test_plot_app_mag_func(testing_data, tmp_path):
    plcm.plot_app_mag_func(pdata=testing_data, z_bin=2.0, drn_out=str(tmp_path))
...
def test_make_color_mag_diagnostic_plots(testing_data, tmp_path):
    plcm.make_color_mag_diagnostic_plots(
        pdata=testing_data,
