...
def get_plotting_data_mock(
    *,
    ...
    if drn_synthetic_cores is not None:
        fn_list_synthetic = glob(os.path.join(drn_synthetic_cores, bnpat))
    else:
        fn_list_synthetic = []
    print(f"Number of synthetic files = {len(fn_list_synthetic)}")
    ...
    if diffstarpop_params is None:
        diffstarpop_params = dpwm.DEFAULT_PARAM_COLLECTION.diffstarpop_params._make(
            metadata["param_collection"].diffstarpop_params
        )
    if mzr_params is None:
        mzr_params = dpwm.DEFAULT_PARAM_COLLECTION.mzr_params._make(
            metadata["param_collection"].mzr_params
        )
    if spspop_params is None:
        spspop_params = dpwm.DEFAULT_PARAM_COLLECTION.spspop_params._make(
            metadata["param_collection"].spspop_params
        )
    if scatter_params is None:
        scatter_params = dpwm.DEFAULT_PARAM_COLLECTION.scatter_params._make(
...
def make_color_mag_diagnostic_plots(
    *,
    ...
    if pdata is None:
        pdict = param_collection._asdict()
