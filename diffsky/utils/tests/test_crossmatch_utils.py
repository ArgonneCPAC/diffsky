...
def test_crossmatch5():
    ...
    xmax = 100
    numx = 10000
    ran_key = jran.key(FIXED_SEED)
    x_key, y_key = jran.split(ran_key, 2)
    x = jran.randint(x_key, minval=0, maxval=xmax + 1, shape=(numx,))

    y = np.arange(-xmax, xmax)[::10]
    y = jran.permutation(y_key, y)
