...
from .. import (
    dbk_sed_kernels_in_situ,
    mc_randoms,
    phot_kernels_in_situ,
    sed_kernels_in_situ,
)
...
def test_dbk_sed_kern_no_einsum(num_halos=20):
    ran_key = jran.key(0)
    lc_data, tcurves = tlcg._get_weighted_lc_photdata_for_unit_testing(
        num_halos=num_halos
    )
    ...
    dbk_sed_kern_results_no_einsum = dbk_sed_kernels_in_situ._dbk_sed_kern_no_einsum(
        phot_randoms,
        dbk_randoms,
        sfh_params,
        lc_data.z_obs,
    ...
    assert np.allclose(
        np.log10(dbk_sed_kern_results_no_einsum.rest_sed),
        np.log10(dbk_sed_kern_results.rest_sed),
        atol=0.1,
    )
