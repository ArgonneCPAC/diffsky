...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "sfh_model", help="Choice of SFH model", choices=DIFFSTARPOP_CALIBRATIONS
    )
    parser.add_argument(
        "-script_name",
    ...
    line_pat += "-fn_u_params sfh_model -sfh_model {6}"
    ...
    with open(fn_submit_script, "w") as fout:
        ...
        for i in patch_list:
            j = i + 1
            line_out = line_pat.format(
                script_name, z_min, z_max, i, j, drn_mock_out, sfh_model
            )
            fout.write(line_out + "\n")
