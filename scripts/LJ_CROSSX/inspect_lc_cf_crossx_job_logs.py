"""Inspect the job logs of lc_cf_crossmatch_script.py"""
...
def check_o_logs(drn):
    ...
    for fn in fn_list:
        ...
        elif "Runtime for script" not in line_collector[-2]:
            bad_jobs.append(fn)
...
def check_for_absent_jobs(drn, istart=0, bn_pat="run_lc_cf_crossx_{0}_to_{1}.sh"):
    n_batch = infer_n_batch(drn, bn_pat="run_lc_cf_crossx_{0}_to_{1}.sh")
    iend = infer_iend(drn, bn_pat="run_lc_cf_crossx_{0}_to_{1}.sh")
    nchar = len(str(iend))
...
def check_for_absent_logs(drn, istart=0, bn_pat="run_lc_cf_crossx_{0}_to_{1}.sh"):
...
def infer_n_batch(drn, bn_pat="run_lc_cf_crossx_{0}_to_{1}.sh"):
    fn_list = sorted(glob(os.path.join(drn, bn_pat.format("*", "*"))))
...
def infer_iend(drn, bn_pat="run_lc_cf_crossx_{0}_to_{1}.sh"):
    fn_list = sorted(glob(os.path.join(drn, bn_pat.format("*", "*"))))
