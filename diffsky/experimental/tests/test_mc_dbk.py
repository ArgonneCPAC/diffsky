...
from .. import mc_dbk as mcsed_dbk2
...
def test_mc_dbk_agrees_with_mc_diffsky_disk_bulge_knot_seds():
    ran_key = jran.key(0)
    lc_data, tcurves = tlcphk._get_weighted_lc_data_for_unit_testing()
    ...
    sed_info2 = mcsed_dbk2._mc_diffsky_seds_dbk_flat_u_params(
        u_param_arr, ran_key, lc_data, DEFAULT_COSMOLOGY, fb
    )

    assert np.allclose(sed_info["obs_mags"], sed_info2["obs_mags"], rtol=1e-4)

    for key in ("obs_mags", "obs_mags_bulge", "obs_mags_disk", "obs_mags_knots"):
        assert np.allclose(sed_info[key], phot_info[key], rtol=1e-4)
        assert np.allclose(sed_info2[key], sed_info[key], rtol=1e-4)
