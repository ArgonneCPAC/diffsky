...
BPOP_DEFAULT_PDICT = OrderedDict(
    ...
    funo_b_logsm_ylo_x0=-11.0,
    funo_b_logsm_ylo_q=0.1,
    funo_b_logsm_ylo_burst=0.4,
    funo_b_logsm_yhi_x0=-11.0,
...
@jjit
def _get_funo_b_from_galpop(logsm, logssfr, fracuno_pop_params):
    (
        funo_b_logsm_x0_x0,
        funo_b_logsm_x0_q,
...
@jjit
def _get_b_from_u_params(logsm, logssfr, lgfuno_pop_u_params):
    lgfuno_pop_params = get_params_from_u_params(lgfuno_pop_u_params)
    funo_b = _get_funo_b_from_galpop(logsm, logssfr, lgfuno_pop_params)
    return funo_b


@jjit
def mc_funobs(ran_key, logsm, logssfr, lgfuno_pop_u_params):
    n_gals = logsm.shape[0]
    a = jnp.zeros(n_gals)
    g = jnp.zeros(n_gals) + LGFUNO_PLAW_SLOPE
    b = _get_b_from_u_params(logsm, logssfr, lgfuno_pop_u_params)
