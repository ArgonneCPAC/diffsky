...
from .. import signdhist_lomem, signdhist
...
def test_nnsig_ndhist_returns_correctly_shaped_results():
    ...
    hiin = loin + 2.0
    result = signdhist_lomem._nnsig_ndhist_vmap(xin, sigin, loin, hiin)
    assert result.shape == (nbins,)


def test_nnsig_ndhist_returns_correct_values_hard_coded_examples():
    ...
    # define ndsig_bin shaped (nbins, ndim) for signdhist_lomem
    ndsig = np.zeros_like(nddata_lo) + 0.001

    result = signdhist_lomem._nnsig_ndhist_vmap(nddata, ndsig, nddata_lo, nddata_hi)
...
def test_nnsig_ndhist_weighted_sum_kern():
    ndim = 3
    ...
    ndlo = np.arange(ndim)
    ndhi = ndlo + 1
    res = signdhist_lomem._nnsig_ndhist_weighted_sum_kern(xin, ndsig, yin, ndlo, ndhi)
    assert res.shape == ()


def test_nnsig_ndhist_weighted_sum_vmap():
    npts, ndim = 200, 3
    ...
    result = signdhist_lomem._nnsig_ndhist_weighted_sum_vmap(
        xin, sigin, yin, ndlo, ndhi
    )
    assert result.sum() == npts
    assert result.shape == (npts,)


def test_nnsig_ndhist_weighted_returns_correctly_shaped_results():
    npts, ndim = 200, 3
    nbins = 5
    ...
    loin = np.zeros((nbins, ndim))
    hiin = loin + 2.0
    result = signdhist_lomem.nnsig_ndhist_weighted(xin, sigin, yin, loin, hiin)
    assert result.shape == (nbins,)


def test_nnsig_ndhist_weighted_returns_correct_values_hard_coded_examples():
    """Manually check a few hard-coded specific examples"""
    xc, yc = -0.5, 0.5
    npts, ndim = 200, 2
    nddata = np.tile((xc, yc), npts).reshape((npts, ndim))
    ...
    result = signdhist_lomem.nnsig_ndhist_weighted(
...
def test_signdhist_lomem_gives_the_same_results_as_signdhist():
    """Manually check a few hard-coded specific examples"""
    xc, yc = -0.5, 0.5
    npts, ndim = 200, 2
    nddata = np.tile((xc, yc), npts).reshape((npts, ndim))
    ...
    # define ndsig_bin shaped (npts, ndim) for signdhist
    ndsig = np.zeros_like(nddata) + 0.001

    signdhist_lomem_result = signdhist_lomem.nnsig_ndhist_weighted(
        nddata, ndsig_bin, ydata, nddata_lo, nddata_hi
    )
    signdhist_result = signdhist.nnsig_ndhist_weighted(
        nddata, ndsig, ydata, nddata_lo, nddata_hi
    )
    correct_result = np.array((0, ydata.sum(), 0))
    assert np.allclose(signdhist_lomem_result, correct_result)
    assert np.allclose(signdhist_lomem_result, signdhist_result)
