...
@jjit
def get_theta_phi_from_ra_dec(ra, dec):
    """Change sky coordinates from {ra, dec} (degrees) to {theta, phi} (radians)"""
    theta = jnp.deg2rad(90.0 - dec)
    phi = jnp.deg2rad(ra)
    return theta, phi
...
def get_timesteps_in_zrange(sim_name, z_min, z_max, max_timestep=MAX_LJ_LC_TIMESTEP):
    _res = hcu.get_timestep_range_from_z_range(sim_name, z_min, z_max)
    timestep_min, timestep_max = _res[2:]
