...
from .. import COSMOS_PARAM_FITS, COSMOS_PARAM_FITS_MERGING
...
def test_cosmos_param_fits_are_ok():
    ...
    msg = "Bounding/unbounding failures in dpw.COSMOS_PARAM_FITS\n"
    if len(bad_params) > 0:
        print(msg)
...
def test_cosmos_param_fits_merging_are_ok():
    """Enforce all parameters in COSMOS_PARAM_FITS_MERGING pass sanity checks encoded in
    dpw.check_param_collection_is_ok function
    ...
    """
    bad_params = []
    for key, param_collection in COSMOS_PARAM_FITS_MERGING.items():
        param_collection_is_ok = dpwm.check_param_collection_is_ok(param_collection)
        if not param_collection_is_ok:
            bad_params.append(key)

    bad_params = list(set(bad_params) - set(KNOWN_COSMOS_CALIBRATION_FAILURES))

    msg = "Bounding/unbounding failures in dpwm.COSMOS_PARAM_FITS_MERGING\n"
    if len(bad_params) > 0:
        print(msg)
        for bad_param in bad_params:
            print(f"{bad_param} fails")
