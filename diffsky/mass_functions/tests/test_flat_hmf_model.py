...
from ..fitting_utils import fit_flat_hmf_model
from ..flat_hmf_model import FLAT_HMF_PARAMS, predict_cuml_hmf
...
def test_lg_hmf_kern_evaluates():
    ...
    redshift = 0.0
    hmf = predict_cuml_hmf(FLAT_HMF_PARAMS, lgmp_arr, redshift)
...
def test_predict_hmf_returns_finite_valued_expected_shape():
    ...
    pred = predict_cuml_hmf(FLAT_HMF_PARAMS, lgmp_arr, redshift)
    assert pred.shape == lgmp_arr.shape
    assert np.all(np.isfinite(pred))

    lgmp = 12.0
    pred = predict_cuml_hmf(FLAT_HMF_PARAMS, lgmp, redshift)
    assert pred.shape == ()
    assert np.all(np.isfinite(pred))
    ...
    lgmp = 12.0
    pred = predict_cuml_hmf(FLAT_HMF_PARAMS, lgmp, zarr)
    ...
    zarr = np.linspace(0, 5, nhalos)
    lgmp_arr = np.linspace(10, 15, nhalos)
    pred = predict_cuml_hmf(FLAT_HMF_PARAMS, lgmp_arr, zarr)
    assert pred.shape == (nhalos,)
    assert np.all(np.isfinite(pred))
...
def test_predict_hmf_accurately_approximates_simulation_data():
    ...
    loss = fit_flat_hmf_model._loss_func_multi_z(FLAT_HMF_PARAMS, loss_data)
    assert loss < 0.01
