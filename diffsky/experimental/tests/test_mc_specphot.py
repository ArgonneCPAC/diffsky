...
def test_mc_lc_phot_changes_with_diffstarpop(num_halos=50):
    ...
    phot_kern_results = mc_specphot.mc_lc_phot(
        ran_key, lc_data, diffstarpop_params=sfh_models["tng"]
    )
    phot_kern_results2 = mc_specphot.mc_lc_phot(
...
def test_mc_lc_sed_is_consistent_with_mc_lc_phot(num_halos=50):
    ran_key = jran.key(0)
    lc_data, tcurves = tmclh._get_weighted_lc_data_for_unit_testing(num_halos=num_halos)
    phot_kern_results = mc_specphot.mc_lc_phot(ran_key, lc_data)
    sed_kern_results = mc_specphot.mc_lc_sed(ran_key, lc_data)
...
def test_mc_lc_dbk_phot(num_halos=50):
    ran_key = jran.key(0)
    lc_data, tcurves = tmclh._get_weighted_lc_data_for_unit_testing(num_halos=num_halos)
    dbk_phot_info = mc_specphot.mc_lc_dbk_phot(ran_key, lc_data)
...
def test_mc_lc_dbk_sed(num_halos=50):
    ran_key = jran.key(0)
    lc_data, tcurves = tmclh._get_weighted_lc_data_for_unit_testing(num_halos=num_halos)
    dbk_sed_info = mc_specphot.mc_lc_dbk_sed(ran_key, lc_data)
