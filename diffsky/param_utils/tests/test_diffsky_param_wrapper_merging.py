...
def test_get_flat_param_names():
    flat_param_names = dpwm.get_flat_param_names()
...
def test_get_param_collection_from_flat_array():
    ...
    assert np.allclose(param_collection[5], default_param_collection[5])
...
def test_unroll_u_param_collection_into_flat_array():
    ...
    u_param_arr = dpwm.unroll_u_param_collection_into_flat_array(*u_param_collection)
    u_param_collection2 = dpwm.get_u_param_collection_from_u_param_array(u_param_arr)

    u_param_arr2 = dpwm.unroll_u_param_collection_into_flat_array(*u_param_collection2)
...
def test_get_param_collection_from_u_param_collection():
    ...
    u_param_collection2 = dpwm.get_u_param_collection_from_param_collection(
        *param_collection
    )
    ...
    assert np.allclose(u_param_collection[5], u_param_collection2[5])


def test_unroll_param_collection_into_flat_array():
    param_arr = dpwm.unroll_param_collection_into_flat_array(
        *dpwm.DEFAULT_PARAM_COLLECTION
    )
    ...
    param_arr2 = dpwm.unroll_param_collection_into_flat_array(*param_collection)

    assert np.allclose(param_arr, param_arr2, rtol=1e-4)


def test_default_diffsky_params_are_ok():
    param_collection_is_ok = dpwm.check_param_collection_is_ok(
        dpwm.DEFAULT_PARAM_COLLECTION
    )
