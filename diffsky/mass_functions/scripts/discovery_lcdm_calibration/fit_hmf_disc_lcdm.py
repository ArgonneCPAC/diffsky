"""This script optimizes the hmf_model.py parameters according to DiscoveryLCDM.
...
disc_lcdm_hmf_params.txt
This ASCII file is programmatically read by hmf_param_reader.py,
and so running this script updates the DiscoveryLCDM hmf_model parameters
...
if __name__ == "__main__":
    ...
    sim_info = get_diffsky_info_from_hacc_sim("DiscoveryLCDM")
    ...
    vol_chunk = Vbox_mpc / NUM_SUBVOLS_DISCOVERY / nchunks
    ...
    fn_disc_lcdm_hmf_redshift_bins = os.path.join(
        drn_testing_data, "disc_lcdm_hmf_redshift_bins.txt"
    )
    np.savetxt(fn_disc_lcdm_hmf_redshift_bins, Z_TABLE)

    fn_disc_lcdm_hmf_logmp_bins = os.path.join(
        drn_testing_data, "disc_lcdm_hmf_logmp_bins.txt"
    )
