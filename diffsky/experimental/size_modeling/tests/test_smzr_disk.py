...
def test_mc_r50_disk_size_behaves_as_expected():
    """Check the returned sizes are within expected bounds"""
    ran_key = jran.key(0)
    n_gals = 20_000
    logsmarr = np.linspace(5, 13, n_gals)
    zarr = np.zeros(n_gals)
    r50_galpop, zscore = smzr_disk.mc_r50_disk_size(logsmarr, zarr, ran_key)
    ...
    assert np.any(r50_galpop > smzr_disk.R50_MAX)

    frac_larger_than_max = np.mean(r50_galpop > smzr_disk.R50_MAX)
    assert frac_larger_than_max < 0.02

    frac_smaller_than_min = np.mean(r50_galpop < smzr_disk.R50_MIN)
    assert frac_smaller_than_min < 0.02


@pytest.mark.skipif(not HAS_SCIPY, reason="Must have scipy installed to run this test")
def test_mc_r50_disk_size_consistent_with_analytic_expectation():
    ...
    median_r50_analytic = 10 ** smzr_disk._lgr50_kern_disk(
        logsmarr, zarr, smzr_disk.DEFAULT_DISK_SIZE_PARAMS
    )
